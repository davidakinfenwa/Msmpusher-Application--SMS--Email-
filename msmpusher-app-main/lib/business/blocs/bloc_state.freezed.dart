...
class _$BlocStateTearOff {
  ...
  Initial<F, T> initial<F, T>() {
    return Initial<F, T>();
  }

  Loading<F, T> loading<F, T>() {
    return Loading<F, T>();
  }

  Success<F, T> success<F, T>({required T data}) {
    return Success<F, T>(
      data: data,
    );
  }

  Error<F, T> error<F, T>({required F failure}) {
    return Error<F, T>(
      failure: failure,
    );
  }
...
abstract class Success<F, T> implements BlocState<F, T> {
  const factory Success({required T data}) = _$Success<F, T>;
...
abstract class Error<F, T> implements BlocState<F, T> {
  const factory Error({required F failure}) = _$Error<F, T>;
