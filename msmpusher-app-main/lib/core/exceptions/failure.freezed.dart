...
class _$FailureTearOff {
  ...
  ServerFailure<T> serverFailure<T>({required ExceptionType<T> exception}) {
    return ServerFailure<T>(
      exception: exception,
    );
  }

  CacheFailure<T> cacheFailure<T>({required ExceptionType<T> exception}) {
    return CacheFailure<T>(
      exception: exception,
    );
  }

  PlatformsFailure<T> platformsFailure<T>(
      {required ExceptionType<T> exception}) {
    return PlatformsFailure<T>(
      exception: exception,
    );
  }
}
...
abstract class ServerFailure<T> implements Failure<T> {
  const factory ServerFailure({required ExceptionType<T> exception}) =
      _$ServerFailure<T>;
...
abstract class CacheFailure<T> implements Failure<T> {
  const factory CacheFailure({required ExceptionType<T> exception}) =
      _$CacheFailure<T>;
...
abstract class PlatformsFailure<T> implements Failure<T> {
  const factory PlatformsFailure({required ExceptionType<T> exception}) =
